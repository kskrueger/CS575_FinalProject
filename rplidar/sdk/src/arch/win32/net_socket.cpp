...
namespace rp {
    namespace net {
        ...
        static const char *_inet_ntop(int af, const void *src, char *dst, int cnt) {
            ...
            if (WSAAddressToStringA((struct sockaddr *) &srcaddr, sizeof(struct sockaddr_storage), 0, dst,
                                    (LPDWORD) & cnt) != 0) {
        ...
        static int _inet_pton(int Family, const char *pszAddrString, void *pAddrBuf) {
            ...
            int result = WSAStringToAddressA((char *) pszAddrString, Family, NULL, (sockaddr * ) & tmpholder,
                                             &actualSize);
            if (result) return -1;
        ...
        size_t SocketAddress::LoopUpHostName(const char *hostname, const char *sevicename,
                                             std::vector<SocketAddress> &addresspool, bool performDNS,
        ...
        u_result SocketAddress::getRawAddress(_u8 *buffer, size_t bufferSize) const {
            switch (getAddressType()) {
                case ADDRESS_TYPE_INET:
                    ...
                    memcpy(buffer, &reinterpret_cast<const sockaddr_in *>(_platform_data)->sin_addr.s_addr,
                           sizeof(reinterpret_cast<const sockaddr_in *>(_platform_data)->sin_addr.s_addr));
                ...
                case ADDRESS_TYPE_INET6:
                    if (bufferSize <
                        ...
                        return RESULT_INSUFFICIENT_MEMORY;
                    memcpy(buffer, reinterpret_cast<const sockaddr_in6 *>(_platform_data)->sin6_addr.s6_addr,
                           sizeof(reinterpret_cast<const sockaddr_in6 *>(_platform_data)->sin6_addr.s6_addr));
        ...
        namespace net {
            ...
            public:
                ...
                virtual StreamSocket *accept(SocketAddress *pairAddress) {
                    ...
                    SOCKET pair_socket = ::accept(_socket_fd, pairAddress
                                                              ? reinterpret_cast<struct sockaddr *>(const_cast<void *>(pairAddress->getPlatformData()))
                                                              : NULL, &addrsize);
                ...
                virtual u_result enableKeepAlive(bool enable) {
                    ...
                    return ::setsockopt(_socket_fd, SOL_SOCKET, SO_KEEPALIVE, (const char *) &bool_true,
                                        (int) sizeof(bool_true)) ? RESULT_OPERATION_FAIL : RESULT_OK;
                }

                virtual u_result enableNoDelay(bool enable) {
                    int bool_true = enable ? 1 : 0;
                    return ::setsockopt(_socket_fd, IPPROTO_TCP, TCP_NODELAY, (const char *) &bool_true,
                                        (int) sizeof(bool_true)) ? RESULT_OPERATION_FAIL : RESULT_OK;
                }
            ...
            public:
                ...
                DGramSocketImpl(SOCKET fd)
                    ...
                    assert(fd >= 0);
                    int bool_true = 1;
                    ::setsockopt(_socket_fd, SOL_SOCKET, SO_REUSEADDR | SO_BROADCAST, (char *) &bool_true,
                                 (int) sizeof(bool_true));
                ...
                virtual u_result recvFrom(void *buf, size_t len, size_t &recv_len, SocketAddress *sourceAddr) {
                    struct sockaddr *addr = (sourceAddr
                                             ? reinterpret_cast<struct sockaddr *>(const_cast<void *>(sourceAddr->getPlatformData()))
                                             : NULL);
